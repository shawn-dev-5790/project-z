...
export default class Particle implements IParticle {
  ...
  renderCrash(ctx: CanvasRenderingContext2D, flag: boolean) {
    ctx.strokeStyle = flag ? 'gold' : 'white'
    ctx.lineWidth = flag ? 10 : 2
  }
  render(ctx: CanvasRenderingContext2D, flag: boolean) {
    if (!this.free) return
    this.renderCrash(ctx, flag)
    ctx.save()
